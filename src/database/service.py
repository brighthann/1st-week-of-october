...
from src.database.models import AlertDB, EndpointStatusDB
...
class DatabaseService:
    ...
    def save_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            db_alert = AlertDB(
                ...
                created_at=datetime.fromisoformat(alert_data.get("timestamp")),
            )
            self.db.add(db_alert)
            self.db.commit()
            logger.info(f"Saved alert for {alert_data.get('endpoint')}")
            return True
        except Exception as e:
    ...
    def get_endpoint_history(
        ...
        try:
            ...
            return (
                self.db.query(EndpointStatusDB)
                .filter(
                    and_(
                ...
                .order_by(EndpointStatusDB.timestamp.desc())
                .all()
            )
        except Exception as e:
    ...
    def get_all_endpoints_history(self, hours: int = 24) -> List[EndpointStatusDB]:
        # Get history for all endpoints
        try:
            cutoff_time = datetime.utcnow() - timedelta(hours=hours)
            return (
                self.db.query(EndpointStatusDB)
                .filter(EndpointStatusDB.timestamp >= cutoff_time)
                .order_by(EndpointStatusDB.timestamp.desc())
                .all()
            )
        except Exception as e:
    ...
    def get_recent_alerts(
        self, endpoint_name: Optional[str] = None, limit: int = 50
    ) -> List[AlertDB]:
        # Get recent alerts
        try:
            query = self.db.query(AlertDB)
            if endpoint_name:
                query = query.filter(AlertDB.endpoint_name == endpoint_name)
            return query.order_by(AlertDB.created_at.desc()).limit(limit).all()
        except Exception as e:
            logger.error(f"Error getting alerts: {e}")
            return []

    def calculate_uptime(self, endpoint_name: str, hours: int = 24) -> float:
    ...
    def cleanup_old_data(self, days: int = 30) -> int:
        ...
        try:
            ...
            # Delete old alerts
            deleted_alerts = (
                self.db.query(AlertDB).filter(AlertDB.created_at < cutoff_time).delete()
            )
