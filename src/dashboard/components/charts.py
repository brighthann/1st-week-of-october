    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                textposition="auto",
                marker_color=[
                    "#00ff00" if up >= 99 else "#ffaa00" if up >= 95 else "#ff0000"
    ...
    fig = go.Figure(
        data=[
            go.Pie(
                ...
                marker_colors=[
                    colors.get(status, "#cccccc") for status in status_counts.index
                ],
            )
        ]
    )

    fig.update_layout(title="Current Status Distribution", template="plotly_white")
